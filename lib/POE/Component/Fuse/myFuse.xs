...
perl_fuse_main(...)
	...
	for(i=0;i<N_CALLBACKS;i++) {
		...
		if(SvOK(var)) {
			croak("invalid callback passed to perl_fuse_main (%s is not a string, code ref, or undef).\n",
				SvPVbyte_nolen(var));
		}
	}
	/*
